  ...
  (
    fun _ => {
      test
        ...
        (fun _ => expect (mapCommandToMove "FBAFB") |> toEqual [F, B, NotValid, F, B]);
      test
        "it moves correctly in a straigth line"
        (fun _ => expect (evaluateMoves movesBasic plutoOne) |> toEqual plutoOneMoved);
      test
        "it goes back to the same place if going backward"
        (fun _ => expect (evaluateMoves moveReturn plutoOne) |> toEqual plutoOne)
    }
  );